...
export class TeamService {
  ...
  getTeamById(id: number): Observable<any> {
    ...
    return this.http.get(`http://localhost:9091/team/getTeamById/${id}`, { headers });
  }
...
if (formDataEntries) {
  for (let pair of formDataEntries) {
    ...
    return this.http.post<Team>('http://localhost:9091/team/addTeam', formData,{ headers });
  }
  ...
  updateTeam(id: number, teamData: Team, imageFiles: File[]): Observable<any> {
    ...
    // Make the HTTP request
    return this.http.put<Team>(`http://localhost:9091/team/updateTeam/${id}`, formData,{ headers });
  }
  deleteTeam(id: number): Observable<any> {
    const headers = new HttpHeaders({
      'Authorization': 'Bearer ' + this.tokenStorageService.getToken()
    });
    return this.http.delete(`http://localhost:9091/team/deleteTeamById/${id}`,{ headers });
  }
  countTeams(): Observable<number> {
    return this.http.get<number>(`http://localhost:9091/team/count`);
  }
}